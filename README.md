...
- 首次访问显示三步可跳过引导：记录 → 觉察 → 关怀；宠物/社群/设备入口；开始第一份记录。
- 完成或跳过后，在本浏览器记住状态，不再自动弹出；顶部「使用指南」可重新打开。
- 引导说明本地保存、导出备份以及模拟功能边界，不创建日记、不自动认养、不读取或上传新数据。
- 新资源：`onboarding.js`、`onboarding.css`，随整个 `public/` 一起部署。

### 实际页面流程导览

首次访问自动启动五步导览：心情选择 → 日记 → 趋势 → 陪伴关怀 → 心情广场。每一步高亮实际功能区，支持下一步、上一步、跳过与 Escape。完成后进入记录页；跳过返回原来的页面。完成状态只存在当前浏览器，顶部使用指南可重新启动。导览不会创建记录、发帖或自动认养。新增 `walkthrough.js` / `walkthrough.css`，部署时随 `public/` 上传。
