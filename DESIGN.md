...
## 慢慢 implementation notes

Adapted from the awesome-design Claude reference. This is a product UI, so existing compact layouts, mobile bottom navigation, routes, all feature controls and pet illustrations are retained. The original JavaScript and storage schemas are unchanged. The final visual layer is public/awesome-theme.css. Chinese editorial headings use the local Songti serif fallback; controls retain the system sans-serif font. Primary terracotta is darkened to #a74d32 for legible white button labels. Semantic mood colors and community accents remain. No custom Anthropic fonts or branding are bundled.
