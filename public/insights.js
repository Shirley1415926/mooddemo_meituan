...
function bindInsightExplorer(){
 ...
 document.querySelectorAll('[data-reflect-entry]').forEach(b=>b.onclick=()=>{const entry=data().find(r=>String(r.id)===b.dataset.reflectEntry);if(!entry)return;beginReflectionFromRecord(entry,b.dataset.reflectFactor);render();focusReflectionStep()});
}
