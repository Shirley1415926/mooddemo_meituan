...
function openDiaryExport(period = 'all') {
  const cutoff=new Date();cutoff.setHours(0,0,0,0);if(period!=='all')cutoff.setDate(cutoff.getDate()-Number(period)+1);
  const entries = records.filter(r=>period==='all'||new Date(r.date)>=cutoff).sort((a,b)=>new Date(b.date)-new Date(a.date));
  if (!entries.length) {toast('这段时间暂无记录，已显示全部日记');if(period!=='all')return openDiaryExport('all');return;}
  openModal(`<span class="section-label">把心情，收藏成一份礼物</span><h2 class="modal-title">留一份，写给自己的日记</h2><p class="modal-copy">整本日记保存为 PDF，或将一篇心情下载成图片。只在当前设备生成。</p><label for="export-period" class="label-row">导出时间范围</label><select id="export-period" class="export-select">${[["all","全部日记"],["7","近 7 天"],["30","近 30 天"]].map(([v,t])=>`<option value="${v}" ${period===v?"selected":""}>${t}</option>`).join('')}</select><div class="export-paper"><span>慢慢 · 私人情绪手记</span><strong>每一种心情，<br>都值得被温柔收藏。</strong><small>${entries.length} 篇记录 · 写给认真生活的自己</small></div><button class="primary full" id="export-pdf">整本日记 · 打印 / 保存为 PDF</button><p class="small-note">在打印窗口中选择「另存为 PDF」。建议使用 A4 纸张，并关闭页眉和页脚。</p><label for="export-entry" class="label-row">选择一篇，保存为图片</label><select id="export-entry" class="export-select">${entries.map((r,i)=>`<option value="${i}">${escape(new Date(r.date).toLocaleDateString('zh-CN'))} · ${moodLabel(r)} · ${escape((r.text||'一份此刻的心情').slice(0,24))}</option>`).join('')}</select><button class="secondary full" id="export-image" style="margin-top:12px">预览并下载 PNG 图片</button><button class="link-button full" id="export-backup" style="margin-top:14px">下载原始数据备份（JSON）</button><p class="small-note center">导出内容包含你的私人日记，分享前请确认。</p>`);
  $('#export-period').onchange=e=>openDiaryExport(e.target.value);
...
function drawDiaryImage(entry) {
  ...
  ctx.fillStyle='#39452d';ctx.font=`500 54px ${font}`;ctx.fillText(moodLabel(entry),padding,310);
